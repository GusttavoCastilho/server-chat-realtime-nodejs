...
io.on("connection", (socket) => {
    ...
    socket.on("join", (data, callback) => {
        ...
        if(error) {
            ...
            socket.emit("message", { user: 'Admin', displayName: `Admin`, text: `Bem vindo a sala, ${addedUser.displayName}`});
            socket.broadcast.to(addedUser.room).emit("message", { user: 'Admin', displayName: `Admin`, text: `${addedUser.displayName} Entrou na sala.`});
    ...
    socket.on("disconnect", () => {
        ...
        if(removedUser) {
            io.to(removedUser.room).emit("message", { user: "Admin", displayName: `Admin`, text: `${removedUser.displayName} Saiu da Sala.`});
